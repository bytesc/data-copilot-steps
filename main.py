...
from ask_ai.ask_ai_for_echart import ask_echart_file, get_ask_echart_file_prompt, get_ask_echart_block_prompt
...
from pywebio.output import put_html, put_text, put_table, put_markdown, put_image
...
def main():
    ...
    while 1:
        ...
        while 1:
            ...
            if isinstance(ans_pd, pd.DataFrame):
                ...
                user_choice = actions("请选择：", [
                    {'label': '接受结果并继续', 'value': 'accept'},
                    {'label': '重新编辑 SQL 代码', 'value': 'retry'}
                ])
        ...
        # 生成图表
        result = ask_py(ans_pd, get_ask_graph_prompt(ask_request),
                        llm, parse_output.assert_png_file, ask_request.retries)
        print(result)

        if result[0] is None:
            while 1:
                # put_text("代码生成失败，生成的代码如下：")
                # put_text(result[4])  # 展示生成的代码
                ...
                try:
                    ...
                    put_text("代码执行成功！")
                    img = open(result[0], 'rb').read()
                    put_image(img)  # 展示生成的图片
                    break
